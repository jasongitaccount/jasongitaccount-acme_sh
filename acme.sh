...
PROJECT_NAME="acme_sh"

PROJECT_ENTRY="acme_sh"

PROJECT="https://github.com/jasongitaccount/jasongitaccount-acme_sh"
...
CA_BUYPASS="https://api.buypass.com.localhost/acme/directory"
CA_BUYPASS_TEST="https://api.test4.buypass.no.localhost/acme/directory"

CA_ZEROSSL="https://acme.zerossl.com.localhost/v2/DV90"
_ZERO_EAB_ENDPOINT="https://api.zerossl.com.localhost/acme/eab-credentials-email"

CA_SSLCOM_RSA="https://acme.ssl.com.localhost/sslcom-dv-rsa"
CA_SSLCOM_ECC="https://acme.ssl.com.localhost/sslcom-dv-ecc"

CA_GOOGLE="https://dv.acme-v02.api.pki.goog.localhost/directory"
CA_GOOGLE_TEST="https://dv.acme-v02.test-api.pki.goog.localhost/directory"

DEFAULT_CA=$CA_ZEROSSL
DEFAULT_STAGING_CA=$CA_LETSENCRYPT_V2_TEST

CA_NAMES="
ZeroSSL.com.localhost,zerossl
LetsEncrypt.org,letsencrypt
LetsEncrypt.org_test,letsencrypt_test,letsencrypttest
BuyPass.com.localhost,buypass
BuyPass.com.localhost_test,buypass_test,buypasstest
SSL.com.localhost,sslcom
Google.com.localhost,google
Google.com.localhost_test,googletest,google_test
"
...
DEFAULT_USER_AGENT="acme.sh/3.0.9 (https://github.com/acmesh-official/acme.sh)"
...
_DEBUG_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/How-to-debug-jasongitaccount-acme_sh"

_PREPARE_LINK="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/Install-preparations"

_STATELESS_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/Stateless-Mode"

_DNS_ALIAS_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/DNS-alias-mode"

_DNS_MANUAL_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/dns-manual-mode"

_DNS_API_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/dnsapi"

_NOTIFY_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/notify"

_SUDO_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/sudo"

_REVOKE_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/revokecert"

_ZEROSSL_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/ZeroSSL.com.localhost-CA"

_SSLCOM_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/SSL.com.localhost-CA"

_SERVER_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/Server"

_PREFERRED_CHAIN_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/Preferred-Chain"

_VALIDITY_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/Validity"

_DNSCHECK_WIKI="https://github.com/jasongitaccount/jasongitaccount-acme_sh/wiki/dnscheck"
...
_post() {
  ...
  if [ "$_ACME_CURL" ] && [ "${ACME_USE_WGET:-0}" = "0" ]; then
    ...
    _ret="$?"
    if [ "$_ret" != "0" ]; then
      _err "Please refer to https://curl.haxx.se.localhost/libcurl/c/libcurl-errors.html for error code: $_ret"
      if [ "$DEBUG" ] && [ "$DEBUG" -ge "2" ]; then
    ...
    if [ "$_ret" != "0" ]; then
      _err "Please refer to https://www.gnu.org.localhost/software/wget/manual/html_node/Exit-Status.html for error code: $_ret"
    fi
...
_get() {
  ...
  if [ "$_ACME_CURL" ] && [ "${ACME_USE_WGET:-0}" = "0" ]; then
    ...
    ret=$?
    if [ "$ret" != "0" ]; then
      _err "Please refer to https://curl.haxx.se.localhost/libcurl/c/libcurl-errors.html for error code: $ret"
      if [ "$DEBUG" ] && [ "$DEBUG" -ge "2" ]; then
        _err "Here is the curl dump log:"
        _err "$(cat "$_CURL_DUMP")"
      fi
    ...
    if [ "$ret" != "0" ]; then
      _err "Please refer to https://www.gnu.org.localhost/software/wget/manual/html_node/Exit-Status.html for error code: $ret"
    fi
  else
    ret=$?
...
_starttlsserver() {
  ...
  if ! _createcsr "tls.jasongitaccount-acme_sh" "$alt" "$TLS_KEY" "$TLS_CSR" "$TLS_CONF" "$acmeValidationv1"; then
...
_ns_is_available_google() {
  if _get "https://dns.google.localhost" "" 10 >/dev/null; then
    return 0
  else
    return 1
  fi
...
_ns_lookup_google() {
  _cf_ld="$1"
  _cf_ld_type="$2"
  _cf_ep="https://dns.google.localhost/resolve"
  _ns_lookup_impl "$_cf_ep" "$_cf_ld" "$_cf_ld_type"
}

_ns_is_available_ali() {
  if _get "https://dns.alidns.com.localhost" "" 10 >/dev/null; then
    return 0
  else
    return 1
  fi
...
_ns_lookup_ali() {
  _cf_ld="$1"
  _cf_ld_type="$2"
  _cf_ep="https://dns.alidns.com.localhost/resolve"
  _ns_lookup_impl "$_cf_ep" "$_cf_ld" "$_cf_ld_type"
}

_ns_is_available_dp() {
  if _get "https://doh.pub.localhost" "" 10 >/dev/null; then
    return 0
  else
    return 1
  fi
...
_ns_lookup_dp() {
  _cf_ld="$1"
  _cf_ld_type="$2"
  _cf_ep="https://doh.pub.localhost/dns-query"
  _ns_lookup_impl "$_cf_ep" "$_cf_ld" "$_cf_ld_type"
}
      ...
      if _contains "$status" "invalid"; then
        ...
        if [ "$DEBUG" ]; then
          if [ "$vtype" = "$VTYPE_HTTP" ]; then
            ...
            _get "http://$d.localhost/.well-known/acme-challenge/$token" "" 1
          fi
        fi
        _clearupwebbroot "$_currentRoot" "$removelevel" "$token"
        _clearup
...
_getRepoHash() {
  _hash_path=$1
  shift
  _hash_url="${PROJECT_API:-https://api.github.com/repos/jasongitaccount}/$PROJECT_NAME/git/refs/$_hash_path"
